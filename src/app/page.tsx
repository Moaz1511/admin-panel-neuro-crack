...
function DesktopSidebar({ user, expanded, onExpand, onLogout }: SidebarProps) {
  return (
    <div className="flex flex-col h-full">
      ...
      {/* <div className={cn(
        "p-6",
        expanded ? "flex items-center gap-4" : "flex flex-col items-center gap-2"
      )}>
      ...
      <nav className="flex-1 px-4">
        {navigationItems.map((item) => (
          <Link
            key={item.name}
            href={item.href}
            className="flex items-center gap-3 px-4 py-3 mb-2 rounded-xl transition-all hover:bg-primary/10 hover:text-primary"
          >
            <item.icon className="h-5 w-5" />
            <span>{item.name}</span>
          </Link>
